...
@section('title', 'Tambah Data Izin Cuti Tahunan')
    ...
    <div class="animated fadeIn">
        ...
        <div class="card">
            <div class="card-header">
                <div class="pull-left">
                    <strong>Tambah Data Izin Cuti Tahunan</strong>
                </div>
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form action="{{ url('izincuti') }}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            is-invalid @enderror">
                                ...
                                @foreach ($pegawais as $item)
                                    <option value="{{ $item->id }}" {{ old('pegawai_id') == $item->id ? 'selected' : null }}>{{ $item->nama_pegawai}}</option>  
                                @endforeach                                
                            </select>
                            @error('pegawai_id')
                            <div class="invalid-feedback">{{ $message }}</div>     
                        ...
                        <div class="form-group">
                            ...
                            is-invalid @enderror">
                                <option value=""></option>
                                @foreach ($pegawais as $item)
                                    <option value="{{ $item->id }}" {{ old('pegawai_id') == $item->id ? 'selected' : null }}>{{ $item->nip}}</option>  
                                @endforeach                                
                            </select>
                            @error('pegawai_id')
                            <div class="invalid-feedback">{{ $message }}</div>     
